...
def test_it_is_lettered_by_hand():
    """A marker face is most of the look; a grotesque reads as a caption."""
    assert P.Params().stage.font in (
        "Marker Felt", "Bradley Hand", "Chalkboard", "Noteworthy",
...
def test_the_phrase_being_sung_is_what_is_drawn(lh):
    ps = lh._phrases(CUES)
    bodies, drawn = lh._spec(ps, ps[0][0][0] + 0.1, 1.0)
    assert drawn > 1
...
def test_nothing_is_drawn_before_the_first_word(lh):
    ps = lh._phrases(CUES)
    _bodies, drawn = lh._spec(ps, ps[0][0][0] - 0.5, 1.0)
    assert drawn == 0
...
def test_a_phrase_clears_rather_than_sitting_through_an_instrumental(lh):
    ...
    ps = lh._phrases(CUES)
    last = ps[-1]
    assert lh._spec(ps, last[-1][0] + lh.LINGER * 0.5, 1.0)[1] > 0
    assert lh._spec(ps, last[-1][0] + lh.LINGER + 1.0, 1.0)[1] == 0
...
def test_the_block_is_written_from_the_lower_left(lh):
    ...
    lh.WAVER = 0.0
    ps = lh._phrases(CUES)
    bodies, _ = lh._spec(ps, ps[0][0][0] + 0.1, 1.0)
...
def test_every_phrase_of_a_real_song_fits_inside_the_frame(lh):
    ...
    assert ps
    for p in ps:
        bodies, _ = lh._spec(ps, p[0][0] + 0.01, 1.0)
...
def test_the_sizes_are_summed_before_they_are_taken_to_the_ink():
    ...
    assert specs["lh_ink"].inputs == [f"lh_sum{n - 1}"]
    for i in range(1, n):
        assert specs[f"lh_sum{i}"].params["operand"] == "add"
...
def test_a_song_with_no_cues_does_not_raise(lh):
    ps = lh._phrases([])
    bodies, drawn = lh._spec(ps, 3.0, 1.0)
