...
def _read_bands(name=BAND_DAT):
    """The measured spectrum, as (times, rows) ready to look up by time.

    Same shape as `_read_drums` and for the same reasons: measured offline, so a
    preview and a render of the same second agree, and looked up by time rather
    than evaluated live, so a frame does not depend on how fast TouchDesigner
    happens to be cooking.

    Returns `([], [])` when there is no table, which every renderer that uses it
    has to survive -- a song ingested before the spectrum existed has none.
    """
    times, rows = [], []
    try:
        d = op(name)
    ...
    for r in range(n):
        try:
            head = d[r, 0].val.strip()
        except (IndexError, AttributeError):
            continue
        if not head or head[0].isalpha():
            continue                      # the header row
        try:
            t = float(head)
            row = tuple(float(d[r, c].val) for c in range(1, cols))
        except (ValueError, IndexError, AttributeError):
            continue
        times.append(t)
        rows.append(row)
...
def _bands_at(times, rows, t):
    """The band levels at `t`, interpolated between the two nearest slices.

    Interpolated rather than nearest, because the table is deliberately coarser
    than the frame rate: stepping between 30 rows a second at 60fps makes every
    bar visibly stair-step.
    """
    n = len(times)
    if not n:
        return ()
